...
import { Download, FileText, Shield, Globe, Hash, Link as LinkIcon } from "lucide-react";
...
export default function PublicBlacklist() {
    ...
    async function fetchData() {
      try {
        ...
        if (filesResponse.ok && statsResponse.ok) {
          ...
          console.error('Failed to fetch data:', filesResponse.status, statsResponse.status);
        }
      } catch (error) {
  ...
  const getIconForType = (type: string) => {
    switch (type.toLowerCase()) {
      ...
      case 'domain': return <FileText className="h-5 w-5" />;
  ...
  const getTypeDescription = (type: string) => {
    switch (type.toLowerCase()) {
      case 'ip': return 'Malicious IP addresses and network ranges';
      case 'domain': return 'Suspicious domains and hostnames';
      case 'hash': return 'File hashes of known malware';
      case 'url': return 'Malicious URLs and web resources';
      default: return 'Threat intelligence indicators';
    }
  };
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white shadow-sm border-b">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex items-center">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900">Public Threat Intelligence Blacklists</h1>
              <p className="text-gray-600">Download the latest threat indicators for your security systems</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="space-y-8">
          {files && Object.entries(files).map(([type, fileList]) => {
            ...
            return (
              ...
              <div className="px-6 py-4 border-b border-gray-200">
                <div className="flex items-center">
                  ...
                  <div>
                    <h2 className="text-xl font-semibold text-gray-900 capitalize">{type} Blacklist</h2>
              ...
              <div className="p-6">
                {safeFileList.length > 0 ? (
                    ...
                    {safeFileList.map((filename: string) => (
                      ...
                      >
                        ...
                        <div className="flex-1">
                          <p className="font-medium text-gray-900 group-hover:text-blue-900">{filename}</p>
                          <p className="text-sm text-gray-500">
                            {stats?.[type.toLowerCase() as keyof BlacklistStats]?.lastUpdated ? 
                  ...
                  <p className="text-gray-500 text-center py-8">No {type.toLowerCase()} blacklist files available</p>
                )}
              </div>
              </div>
            );
        ...
        <div className="mt-12 bg-blue-50 rounded-lg p-6">
          <h3 className="text-lg font-medium text-blue-900 mb-2">Usage Information</h3>
          <div className="text-blue-800 space-y-2">
            <p>• Files are updated automatically from various threat intelligence sources</p>
            <p>• Download and integrate these lists into your security tools and firewalls</p>
            <p>• Files are provided in plain text format, one indicator per line</p>
            <p>• Check back regularly for the latest threat intelligence updates</p>
          </div>
        </div>
      </div>
    </div>
